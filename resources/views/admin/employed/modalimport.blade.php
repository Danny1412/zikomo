...
<div class="mt-10 mb-5 ml-5">
    <button class="px-4 py-2 font-bold text-gray-500 bg-transparent border border-gray-500 rounded-full modal-open hover:border-indigo-500 hover:text-indigo-500" onclick="my_modal_1.showModal()">Importar Empleados</button>
</div>
    <dialog id="my_modal_1" class="rounded modal">
    <div class="px-10 py-10 modal-box">
        <h3 class="mb-3 text-lg font-bold">Importar Empleados</h3>
        <div class="modal-action">
            <form action="{{ route('importar.empleados') }}" method="post" enctype="multipart/form-data">
                @csrf
